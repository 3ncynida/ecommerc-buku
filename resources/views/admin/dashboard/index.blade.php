        ...
        <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <h4 class="text-2xl font-bold text-amber-600">{{ $orders->where('payment_status', 'pending')->count() }}
                    </h4>
                </div>
            ...
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <h4 class="text-2xl font-bold text-emerald-600">
                        {{ $orders->where('payment_status', 'success')->count() }}</h4>
                </div>
        ...
        <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <h4 class="text-2xl font-bold text-blue-600">{{ $orders->whereIn('item_status', ['pending', 'diproses'])->count(); }}
                    </h4>
                </div>
                    ...
                    @foreach ($orders as $order)
                        <tr class="hover:bg-gray-50 transition">
                            ...
                            <td class="px-6 py-4 text-center">
                                ...
                                @else
                                    <span
                                        class="inline-flex items-center px-3 py-1 rounded-full text-xs font-bold bg-rose-100 text-rose-700">
                            ...
                            <td class="px-6 py-4 text-center">
                                <form action="{{ route('admin.orders.update-status', $order->id) }}" method="POST"
                                    ...
                                    <select name="item_status" onchange="this.form.submit()"
                                        ...
                                        <option value="pending" {{ $order->item_status == 'pending' ? 'selected' : '' }}>
                                            Pending</option>
                                        <option value="diproses" {{ $order->item_status == 'diproses' ? 'selected' : '' }}>
                                            Diproses</option>
                                        <option value="dikirim" {{ $order->item_status == 'dikirim' ? 'selected' : '' }}>
                                            Dikirim</option>
                                        <option value="selesai" {{ $order->item_status == 'selesai' ? 'selected' : '' }}>
                                            Selesai</option>
                                    </select>
                                </form>
                            </td>
