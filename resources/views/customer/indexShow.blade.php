        ...
        <div class="flex flex-col md:flex-row gap-12">
            ...
            <div class="md:w-2/3">
                ...
                <div class="flex space-x-6 mb-8">
                    <button class="flex items-center text-gray-500 hover:text-red-500 transition">
                        <i class="fa-regular fa-heart mr-2 text-xl"></i> Favorit
                    </button>
                ...
                <div class="sticky bottom-6 mt-12 md:relative md:bottom-0">
                    <form action="{{ route('cart.add', $item->id) }}" method="POST" class="flex gap-4">
                        @csrf
                        <div class="flex items-center bg-white border-2 border-gray-200 rounded-xl overflow-hidden shrink-0">
                            ...
                            <input type="number" name="quantity" value="1" class="w-12 text-center border-none focus:ring-0 font-bold" min="1">
                            <button type="button" class="px-4 py-2 hover:bg-gray-100 font-bold">+</button>
                        </div>
                        <button class="flex-1 bg-indigo-600 hover:bg-indigo-700 text-white py-4 rounded-xl font-bold text-lg shadow-xl shadow-indigo-100 transition-all flex items-center justify-center gap-3">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-6">
                ...
                <a href="{{ route('book.show', $related->slug) }}" class="group">
                    <div class="bg-white rounded-xl overflow-hidden transition group-hover:translate-y-[-5px]">
                        <img src="{{ asset('storage/' . $related->image) }}" class="w-full h-64 object-cover rounded-xl shadow-sm border">
                        <div class="mt-3">
                            <p class="text-xs text-gray-400 mb-1">{{ $related->author->name ?? 'Penulis' }}</p>
                            <h4 class="font-bold text-gray-900 truncate text-sm group-hover:text-indigo-600 transition">{{ $related->name }}</h4>
                            <p class="text-indigo-600 font-bold mt-1">Rp{{ number_format($related->price, 0, ',', '.') }}</p>
                        </div>
                    </div>
                </a>
                @endforeach
