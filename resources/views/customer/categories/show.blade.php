                ...
                <form action="{{ url()->current() }}" method="GET" id="sortForm">
                    ...
                    <div class="relative inline-block w-64">
                        <select name="sort" onchange="this.form.submit()"
                            ...
                            <option value="termurah" {{ request('sort') == 'termurah' ? 'selected' : '' }}>Harga: Termurah
                            </option>
                            <option value="termahal" {{ request('sort') == 'termahal' ? 'selected' : '' }}>Harga: Termahal
                            </option>
                        </select>
                ...
                @forelse($items as $book)
                    <div
                        ...
                        <div class="relative aspect-[3/4.2] mb-5 overflow-hidden rounded-[1.5rem] bg-gray-50">
                            ...
                            @if($book->stok <= 0)
                                <div class="absolute inset-0 bg-white/60 backdrop-blur-[2px] flex items-center justify-center p-4">
                                    <span
                                        class="bg-white text-red-600 text-[10px] font-black px-3 py-1.5 rounded-full shadow-lg border border-red-100 uppercase tracking-widest">
                                        Stok Habis
                                    </span>
                                </div>
                            @endif
                        ...
                        <div class="space-y-1">
                            ...
                            <h3
                                class="text-sm font-bold text-gray-900 line-clamp-2 leading-snug h-10 group-hover:text-indigo-600 transition">
                                {{ $book->name }}
                            </h3>
                            <div class="pt-2">
